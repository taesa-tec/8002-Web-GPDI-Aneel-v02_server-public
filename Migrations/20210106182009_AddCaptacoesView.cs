...
{
    public partial class AddCaptacoesView : Migration
    {
...
SELECT
       ...
       COUNT(DISTINCT PF.Id) as TotalPropostas,
       C.CreatedAt,
       C.EnvioCaptacao,
       C.Termino,
       C.Cancelamento
...
LEFT JOIN PropostaFornecedor PF on C.Id = PF.CaptacaoId  AND PF.Finalizado = 1 AND PF.Participacao = 1
GROUP BY C.Id, C.Titulo, C.CriadorId, ANU.NomeCompleto, C.CreatedAt, C.EnvioCaptacao, C.Termino, C.Cancelamento, C.Status, C.UsuarioSuprimentoId, A.NomeCompleto;";
        ...
        {
            migrationBuilder.Sql("DROP VIEW IF EXISTS CaptacoesView");
        }
    }
}