...
namespace APIGestor.Authorizations {
    public class ProjectAuthorizationHandler : AuthorizationHandler<ProjectAccessRequirement, UserProjeto> {
        protected override Task HandleRequirementAsync( AuthorizationHandlerContext context, ProjectAccessRequirement requirement, UserProjeto resource ) {
            var userPermission = resource.CatalogUserPermissao.Valor;
            if(ProjectPermissions.NiveisValues.ContainsKey(userPermission) && (ProjectPermissions.NiveisValues[userPermission] & requirement.AccessValue) == requirement.AccessValue) {
    ...
    public static class ProjectPermissions {
        ...
        public static ProjectAccessRequirement LeituraEscrita = new ProjectAccessRequirement((int)(Access.Leitura | Access.Escrita));
        public static ProjectAccessRequirement Aprovador = new ProjectAccessRequirement((int)(Access.Leitura | Access.Escrita | Access.Aprovador));
        public static ProjectAccessRequirement Administrator = new ProjectAccessRequirement((int)(Access.Leitura | Access.Escrita | Access.Aprovador | Access.Administrador));
    }
}
