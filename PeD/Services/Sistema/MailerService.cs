...
namespace PeD.Services.Sistema
{
    public class MailerService
    {
        ...
        public Task<Response> SendMail(ApplicationUser user, string subject, string file,
        ...
        public Task<Response> SendMailBase(ApplicationUser user, string subject, string Content,
            (string text, string url) action = default((string text, string url)))
        {
            ...
            if (!String.IsNullOrEmpty(action.text) && !String.IsNullOrEmpty(action.url))
            {
                ...
                Action = String.Format(
                    "<a href=\"{0}\" style=\"display: inline-block; background: #042769; font-weight: bold; color:#FFF; border-radius: 4px; padding: 8px 16px; text-decoration: none; text-transform: uppercase;\">{1}</a>",
                    url, action.text);
            }
            ...
            var userTesteA = new ApplicationUser()
                {Email = "diego.franca@lojainterativa.com", NomeCompleto = "Diego França"};
            var userTesteB = new ApplicationUser()
                {Email = "bruno.galindo@lojainterativa.com", NomeCompleto = "Bruno Galindo"};
            var userTesteC = new ApplicationUser()
                {Email = "juliano.caldeira@taesa.com.br", NomeCompleto = "Juliano Caldeira"};
