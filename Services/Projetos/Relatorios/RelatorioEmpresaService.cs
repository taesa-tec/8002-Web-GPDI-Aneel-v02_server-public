...
namespace APIGestor.Services.Projetos.Relatorios
{
    public class RelatorioEmpresaService
    {
        ...
        public RelatorioEmpresaService(GestorDbContext context, AlocacaoRhService alocacaoRhService,
            AlocacaoRmService alocacaoRmService, RegistroFinanceiroService registroFinanceiroService)
        {
        ...
        {
            List<RegistroFinanceiro> registroFinanceiros =
                RegistroFinanceiroService.ListarTodos(projetoId, StatusRegistro.Aprovado).ToList();
                ...
                foreach (var relatorio in empresa.Relatorios)
                {
                    foreach (var item in relatorio.Items)
                    {
                        ...
                        if (item.RecursoHumano != null)
                        {
                            ...
                            newItem.EntidadeRecebedora = (item.RecursoHumano.Empresa.Cnpj == null)
                                ? item.RecursoHumano.Empresa.CatalogEmpresa.Nome
                                : item.RecursoHumano.Empresa.RazaoSocial;
                            newItem.CnpjEntidadeRecebedora = (item.RecursoHumano.Empresa.Cnpj == null)
                                ? null
                                : item.RecursoHumano.Empresa.Cnpj;
                        ...
                        if (item.AlocacaoRh != null)
                        {
                            ...
                            newItem.EntidadePagadora = (item.AlocacaoRh.Empresa.Cnpj == null)
                                ? item.AlocacaoRh.Empresa.CatalogEmpresa.Nome
                                : item.AlocacaoRh.Empresa.RazaoSocial;
                        ...
                        if (item.AlocacaoRm != null)
                        {
                            ...
                            newItem.EntidadePagadora = (item.AlocacaoRm.EmpresaFinanciadora.Cnpj == null)
                                ? item.AlocacaoRm.EmpresaFinanciadora.CatalogEmpresa.Nome
                                : item.AlocacaoRm.EmpresaFinanciadora.RazaoSocial;
                            newItem.CnpjEntidadePagadora = (item.AlocacaoRm.EmpresaFinanciadora.Cnpj == null)
                                ? null
                                : item.AlocacaoRm.EmpresaFinanciadora.Cnpj;
                            ;
                            newItem.EntidadeRecebedora = (item.AlocacaoRm.EmpresaRecebedora == null)
                                ? null
                                : (item.AlocacaoRm.EmpresaRecebedora.Cnpj == null)
                                    ? item.AlocacaoRm.EmpresaRecebedora.CatalogEmpresa.Nome
                                    : item.AlocacaoRm.EmpresaRecebedora.RazaoSocial;
                            newItem.CnpjEntidadeRecebedora = (item.AlocacaoRm.EmpresaRecebedora == null) ? null :
                                (item.AlocacaoRm.EmpresaRecebedora.Cnpj == null) ? null :
                                item.AlocacaoRm.EmpresaRecebedora.Cnpj;
                        }
        ...
        public MemoryStream ExportarRelatorio(List<RelatorioEmpresaCsv> data, string tipo)
        {
            ...
            TextWriter tw = new StreamWriter(mr, Encoding.UTF8);
            var csv = new CsvWriter(tw, CultureInfo.CurrentCulture);
        ...
        public XLWorkbook gerarXLSOrcamento(int projetoId)
        {
            ...
            {
                table.Rows.Add(item.Nome, item.Titulo, item.Cpf, item.Empresa, item.EmpresaRecebedora, item.Etapa,
                    item.ValorHora, item.HorasTotais, item.custo, item.UrlCurriculo);
            }
            ...
            {
                string atividade = i.RecursoMaterial.Atividade != null
                    ? i.RecursoMaterial.Atividade.Nome
                    : String.Empty;
        ...
        public XLWorkbook gerarXLSExtrato(int projetoId)
        {
            ...
            {
                tableEmpresas.Rows.Add(empresa.Nome, empresa.Total, empresa.TotalAprovado, empresa.Valor,
                    empresa.ValorAprovado, empresa.Desvio / 100m);
            }
            ...
            {
                "Nome completo", "CPF", "Função", "Empresa", "Empresa Financiadora", "Empresa Recebedora", "Qtd Hrs",
                "Custo Hora", "Custo Total", "Mês"
            })
            {
            ...
            foreach (var col in new string[]
            {
                "Nome Item", "Recurso", "Categoria", "Especificação", "Empresa Financiadora", "Empresa Recebedora",
                "Mês", "Qtd Itens", "Valor Unitário", "Valor Total"
            })
            {
