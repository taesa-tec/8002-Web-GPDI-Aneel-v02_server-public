...
{
    public class UserProjetoService : BaseService
    {
        public ProjetoService projetoService;

        public UserProjetoService(ProjetoService service, GestorDbContext context, IAuthorizationService authorization,
            LogService logService) : base(context, authorization, logService)
        {
        ...
        public List<UserProjeto> ListarTodos(string userId, int statusId = 0)
        {
            var UserProjetos = _context.UserProjetos
                .Where(y => y.UserId == userId)
                .Where(p => statusId == 0 || p.Projeto.CatalogStatusId == statusId)
        ...
        public Resultado IncluirProjeto(List<UserProjeto> dadosUserProjeto)
        {
            ...
            foreach (UserProjeto dados in dadosUserProjeto)
            {
                ...
                if (resultado.Inconsistencias.Count == 0)
                {
                    ...
                    if (UserProjeto != null)
                    {
                        _context.UserProjetos.RemoveRange(
                            _context.UserProjetos.Where(t => t.ProjetoId == dados.ProjetoId));
                    }
